...
export const SceneLogoIntro: React.FC = () => {
    ...
    // Phase 2: Slide Left & Reveal Text
    const SLIDE_START = 45;
    ...
    // Pictogram Animation
    // Logo text spans roughly x=40 to x=155.
    // Pictogram is ~33px wide.
    // We want the Picto to START far right (covering the end of text) and slide LEFT to x=0.
    // Start position: x=115 (so Picto right edge is at ~148, covering end of text).
    // End position: x=0 (normal logo position).
    const pictogramX = interpolate(slideProgress, [0, 1], [115, 0]);

    // Text Reveal Mask
    // We want to reveal text BEHIND the pictogram as it moves left.
    // "Behind" means to the RIGHT of the pictogram.
    // Mask X should follow the right edge of the pictogram.
    // Picto right edge = pictogramX + 33 (picto width).
    // At start: Picto at 115, right edge at 148. Mask X = 148.
    // At end: Picto at 0, right edge at 33. Mask X = 33.
    // Mask width = 155 - maskX (covers everything to the right).
    const maskX = interpolate(slideProgress, [0, 1], [160, 33]);

    // Global Centering Adjustment
    // At start, visible object is Pictogram at x=115 (center ~130).
    // At end, visible object is Full Logo (center ~77.5).
    // To keep the visual center stable, we shift the container.
    // Start shift: 77.5 - 130 = -52.5.
    // End shift: 0.
    const globalX = interpolate(slideProgress, [0, 1], [-52.5, 0]);
    ...
    const overallScale = popIn * exitScale;

    // Black Background Overlay
    // Fades out once the logo is fully revealed (around frame 90-100)
    const BG_FADE_START = 45;
                ...
                >
                    <defs>
                        <mask id="textRevealMask">
                            {/* Mask reveals everything to the RIGHT of maskX */}
                            <rect x={maskX} y="0" width={155 - maskX + 10} height="47" fill="white" />
                        </mask>
                    </defs>
                    ...
                    <g style={{ transform: `translateX(${globalX}px)` }}>
                        ...
                        {/* Part 2: Text (White) - Stationary, revealed by mask */}
                        <g mask="url(#textRevealMask)">
                            <g>

...
interface LargeNotificationUIProps {
    startFrame?: number;
    className?: string; // Add className prop
}

// Icon components (reused for consistency - SCALED UP)
const HomeIcon = ({ active }: { active?: boolean }) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill={active ? "#0078FF" : "currentColor"}>
...
export const LargeNotificationUI: React.FC<LargeNotificationUIProps> = ({
    startFrame = 0,
    className = "",
}) => {
        ...
        >
                ...
                >
                    ...
                    <div className="w-[50%] h-full flex flex-col relative">
                        ...
                        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#0f1823]">
                            <span className="font-semibold text-lg">Questions en attente</span>
                            <span className="text-sm px-3 py-1.5 rounded-full bg-[#0078FF]/20 text-[#0078FF]">
                        ...
                        <div className="flex-1 flex flex-col gap-4 px-6 py-6">
                            {notifications.map((notif, index) => {
                                ...
                                return (
                                    <div
                                        key={notif.id}
                                        className="bg-[#1e2a39] rounded-2xl p-6 border border-white/5 relative overflow-hidden group"
                                        style={{
                    ...
                    <div className="w-[50%] h-full flex flex-col relative">
                        {/* Header 2 */}
                        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-[#0f1823]">
                            <span className="font-semibold text-lg">Répondre</span>
                            <div className="w-8" />
                        </div>

                        <div className="flex-1 flex flex-col gap-6 px-6 py-6">
                            {/* Question Card */}
                            <div className="bg-[#1e2a39] rounded-2xl p-6 border border-white/5">
                                <p className="text-sm text-white/50 mb-3">Question posée</p>
                                <p className="text-lg font-medium leading-snug">{notifications[0].question}</p>
                            </div>

                            {/* Response Area */}
                            <div className="flex-1 bg-[#1e2a39] rounded-2xl p-6 border border-white/5 flex flex-col">
                                <p className="text-sm text-white/50 mb-4">Votre réponse</p>
                            ...
                            <div className="flex justify-center py-3">
                                <button
                                    className="w-16 h-16 rounded-full bg-[#1e2a39] border border-white/10 flex items-center justify-center text-white/50"
                                >
                            ...
                            {/* Submit Button */}
                            <button
                                className={`w-full py-4 rounded-2xl font-medium text-base transition-all duration-200 relative overflow-hidden ${localFrame > TAP_SUBMIT_FRAME ? "bg-[#22c55e]" : "bg-[#0078FF]"}`}
                            >
            ...
            <div className="flex justify-around py-4 border-t border-white/10 pb-8 bg-[#0f1823]">
                {[
                    ...
                    <div
                        key={item.label}
                        className={`flex flex-col items-center gap-1.5 ${item.active ? "text-[#0078FF]" : "text-white/40"
                            }`}
                    >
